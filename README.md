...
- `migrations/20260727194500_create_public_order.sql` — безопасное создание
  заказа гостем: состав, варианты, наличие и итоговая цена повторно
  рассчитываются в PostgreSQL;
- `migrations/20260727203000_expand_checkout.sql` — расширенное оформление:
  получатель, открытка, доставка/самовывоз, интервалы, антиспам, статусы и
  безопасная публичная страница успешного заказа;
...
После обновления проекта обязательно выполните `npx supabase db push`: миграция
`20260727203000_expand_checkout.sql` добавляет поля оформления и безопасную
функцию страницы успешного заказа.
...
- корзина сохраняется в браузере без регистрации; перед созданием заказа
  сервер повторно проверяет опубликованность товаров, варианты, наличие и
  цены, не используя сумму, переданную браузером;
- онлайн-оплата подключается отдельным провайдером: форма не запрашивает и не
  хранит реквизиты банковских карт; до этого менеджер подтверждает оплату;
