...
- `migrations/20260728120000_managed_delivery_zones.sql` — управляемые зоны
  Москвы и области с серверным расчётом доставки;
- `migrations/20260729120000_payment_architecture.sql` — отдельные платежи,
  журнал событий, идемпотентные webhook и безопасная страница статуса;
...
### Управляемые зоны доставки

Тарифы доставки не записаны в интерфейсе сайта. Они хранятся в таблице
`delivery_zones` и меняются владельцем в разделе `/admin/delivery`.

Начальный набор включает самовывоз, девять округов Москвы внутри МКАД,
двадцать городов Московской области и индивидуальный расчёт. Если введённого
города нет среди настроенных, оформление предлагает индивидуальную зону с
сообщением «Стоимость уточнит менеджер».

При создании заказа сервер повторно получает активную зону, проверяет
минимальную сумму, срочную доставку и временной интервал. В заказ сохраняется
снимок тарифа, поэтому последующее изменение зоны не меняет старый заказ.

### Подготовка онлайн-оплаты

Платёжная архитектура не привязана к одному банку. Общий интерфейс
`PaymentProvider` поддерживает банковскую карту и СБП через выбранного
провайдера. Сейчас безопасно доступен только `MockPaymentProvider`; адаптеры
ЮKassa и Т-Банка являются намеренно неактивными заготовками и не выполняют
сетевые запросы.

Заказ и попытки оплаты хранятся отдельно. Сумма каждой попытки берётся сервером
из `orders.total_kopecks`, а повторный webhook применяется вместе с записью в
`payment_events` в одной транзакции. Возврат покупателя на страницу статуса не
меняет оплату.

Настройка mock-режима, перечень данных от провайдеров и чек-лист перед реальным
запуском находятся в [`docs/PAYMENTS.md`](docs/PAYMENTS.md).
...
- онлайн-оплата подключается отдельным провайдером: форма не запрашивает и не
  хранит реквизиты банковских карт; реальная оплата выключена, а ссылку можно
  создать только для подтверждённого менеджером заказа с окончательной суммой;
